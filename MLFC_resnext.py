...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.downsample = downsample
        self.stride = stride
        self.Cluster= Cluster(64)
        self.General  = General(planes * self.expansion)
    ...
    def forward(self, x):
        ...
        out1 = out
        out2 = out
        out1 = self.General(out1)
        out2 = self.Cluster(out2)         
        out = out * 0.3 + out1 * 0.5 + out2 * 0.2
...
def resnext50_32x4d(pretrained=False, NumClass=45 , **kwargs):
    ...
    if pretrained==True:
            mlfc_resnext50 = './data.pth'
            pretrained_dict = torch.load(mlfc_resnext50)
            Num = NumClass
            num_fits = model.fc.in_features
            model.fc = nn.Linear(num_fits, Num)
